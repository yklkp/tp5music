(function($){
	...
	// for (var i = 0; i < playlist.length; i++){
		// var item = playlist[i];
		// $('#playlist').append('<li class="lib" style="overflow:hidden;"><strong style="margin-left: 5px;"><a href="{/index/index/play}">'+item.title+'</a></strong><span style="float: right;" class="artist">'+item.artist+'</span></li>');
		// if (item.mp3 == "") {
	...
	var play = function(){
		...
		timeout = setInterval(updateProgress, 0);
		//超过显示栏运行跑马灯
		if(isExceedTag()) {
			if (isInitMarquee) {
				initMarquee();
